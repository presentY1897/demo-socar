...
- **부름(탁송) 수령**: 지도에서 위치를 지정하면 차량을 배달 — 직전 반납 + A* 탁송 시간 기반 가용성 판정 ([ADR-006](docs/adr/006-bureum-delivery.md))
...
pnpm --filter @socar/api test:int   # 통합 13케이스: 동시 예약 경합, 연장 충돌, 편도 존 이동, 부름 제약, 변경 차액 (DB 필요)
```
