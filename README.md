...
- **이용 플로우** — 체크인(촬영) → 가상 스마트키 → 이용 중(연장·사고 접수) → 체크아웃(촬영) → 반납·정산.
  ...
  화면은 한 번에 한 단계만 보여주고, 이용 중에는 홈이 지도 대신 이용 화면
...
```bash
pnpm test                           # 단위 170 + 프론트 259 — DB·서버 없이 돈다
pnpm --filter @socar/api test:int   # 통합 293 — 실제 PostgreSQL 필요
```

| 층 | 수 | 도구 | 무엇을 지키나 |
|---|--:|---|---|
| 단위 | 170 | Vitest · Jest | 요금·정산, 권한 매핑, 상태 머신 같은 순수 함수 규칙 |
| 프론트 | 259 | Vitest + RTL + MSW | 역할·등급별 화면 분기와 화면 동선 |
| 통합 | 293 | Supertest + 실제 PostgreSQL | 동시성·트랜잭션·권한 가드·E2E 동선 |
