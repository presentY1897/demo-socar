...
```bash
pnpm test                     # 단위 66 + 프론트 51 — DB·서버 없이 돈다
pnpm --filter @socar/web test # 프론트 51 — MSW 목 서버로 API를 대신한다 (서버·DB 불필요)
pnpm --filter @socar/api test:int   # 통합 58 (DB 필요)
```

**단위 66** — 요금/정산 엔진 22 · 사진 스키마 19 · 배차 스코어링 8 · 차종 매뉴얼 6 ·
스마트키 상태 머신 5 · 편도 위치 체인 4 · 사진 저장 변환 2

**프론트 51** (Vitest + RTL + MSW) — 예약 상세 단계형 흐름 8 · 예약 변경 패널 7 ·
시간 선택 6 · 사고 접수 5 · 사진 압축 5 · 사진 촬영 UI 4 · 스마트키 패널 4 ·
차종 매뉴얼 4 · 문의 4 · 홈(존 검색) 4

**통합 58** (Supertest + 실제 PostgreSQL) —
**M1 전체 동선 E2E 12**(한 예약으로 예약 생성 → 시각·반납 존 변경 → 체크인 → 스마트키 →
매뉴얼 → 연장 → 체크아웃 → 반납·정산 완주 + 게이트 위반 3종) ·
반납 존 변경 10 · 부름 제약 6 · 예약 동시성/멱등성 5 · 체크인·체크아웃 게이트 5 ·
스마트키 5 · 차종 매뉴얼 5 · 문의·사고 접수 4 · 연장 충돌·편도 존 이동·변경 차액 3 ·
도메인 스모크 2 · 사진 DB 왕복 1
