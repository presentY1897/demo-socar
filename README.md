...
```bash
pnpm test                     # 단위 31케이스: 요금 엔진·배차 스코어링·편도 위치 체인
pnpm --filter @socar/api test:int   # 통합 8케이스: 동시 예약 경합, 연장 충돌, 편도 존 이동, 변경 차액 (DB 필요)
```

## 스코프 아웃 (의도적 결정)

부름(차량 배달) · 패스포트 구독 · 하이패스 통행료 후불 합산 · 실사고/보험 청구 플로우는
데모 범위에서 제외했다. 실제 상품과 이 데모의 관계는 [ADR-003](docs/adr/003-dispatch-scoring.md),
[ADR-005](docs/adr/005-oneway-vehicle-location.md)에 명시.
