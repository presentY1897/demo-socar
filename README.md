...
- SSE 실시간 차량 현황
- **핸들러 작업 배정**(`/ops/tasks`) — 상태·타입·기한으로 좁혀 보고, 후보 핸들러를
  **점수가 아니라 근거**(마지막 완료 지점에서의 거리, 진행 중인 작업 수)와 함께 제시.
  존 간 **재배치 작업**도 여기서 직접 낸다
...
```bash
pnpm test                     # 단위 105 + 프론트 117 — DB·서버 없이 돈다
pnpm --filter @socar/web test # 프론트 117 — MSW 목 서버로 API를 대신한다 (서버·DB 불필요)
pnpm --filter @socar/api test:int   # 통합 229 (DB 필요)
```

**단위 105** — 요금/정산 엔진 22 · 사진 스키마 19 · 법인 등급·권한 매핑 14 ·
리스 계산(만기 D-day·이용률) 13 · 배차 스코어링 8 · **핸들러 작업 전이 규칙 8** ·
차종 매뉴얼 6 · 스마트키 상태 머신 5 · 편도 위치 체인 4 · **배정 후보 추천 4** ·
사진 저장 변환 2

**프론트 117** (Vitest + RTL + MSW) — 등급별 화면 분기 매트릭스 21 · 법인 플릿/리스 11 ·
예약 상세 단계형 흐름 8 · 예약 변경 패널 7 · **핸들러 작업 큐 7** · 시간 선택 6 ·
사고 접수 5 · 사진 압축 5 · 멤버 등급 관리 5 · 법인 배차 5 · 리스 요청 처리(운영) 5 ·
사진 촬영 UI 4 · 스마트키 패널 4 · 차종 매뉴얼 4 · 비즈니스 셸 4 · 문의 4 · 홈(존 검색) 4 ·
내비 역할 분기 2 · 타임라인 보드 2 · 구 경로 리다이렉트 2 · **로그인 데모 계정 2**

**통합 229** (Supertest + 실제 PostgreSQL) —
...
반납 존 변경 10 · 법인 배차 추천/승인 8 · **작업 배정 API 7** · **핸들러 작업 API 7** ·
부름 제약 6 · 리스 도메인 백필 6 · 예약 동시성/멱등성 5 · 체크인·체크아웃 게이트 5 ·
스마트키 5 · 차종 매뉴얼 5 · **핸들러 도메인 5** · 문의·사고 접수 4 ·
**작업 자동 생성 트리거 4** · 연장 충돌·편도 존 이동·변경 차액 3 · 도메인 스모크 2 ·
사진 DB 왕복 1
