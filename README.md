...
```bash
pnpm test                     # 단위 136 + 프론트 172 — DB·서버 없이 돈다
pnpm --filter @socar/web test # 프론트 172 — MSW 목 서버로 API를 대신한다 (서버·DB 불필요)
pnpm --filter @socar/api test:int   # 통합 258 (DB 필요)
```

**단위 136** — 요금/정산 엔진 22 · 사진 스키마 19 · **텔레메트리 모의 엔진 14** ·
...
**프론트 172** (Vitest + RTL + MSW) — 등급별 화면 분기 매트릭스 21 ·
**운영 센터 홈·차량 탭 16** · **운영 센터 존/계약·고객 탭 13** ·
**운영 센터 작업/배차·회계 탭 12** · 법인 플릿/리스 11 · 예약 상세 단계형 흐름 8 ·
예약 변경 패널 7 · 핸들러 작업 큐 7 · 시간 선택 6 · **작업 보드 분할 5** ·
**Fleet 정렬·필터 5** · 사진 압축 5 · 사고 접수 5 · 리스 요청 처리(운영) 5 ·
...
**통합 258** (Supertest + 실제 PostgreSQL) —
